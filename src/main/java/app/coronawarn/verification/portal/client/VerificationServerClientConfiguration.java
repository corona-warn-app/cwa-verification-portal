...
public class VerificationServerClientConfiguration {
  ...
  public Client feignClient() {
    boolean isMutualAuthEnabled = Boolean.parseBoolean(
      environment.getProperty("cwa-verification-server.ssl.two-way-authentication-enabled"));
  ...
  private SSLSocketFactory getSslSocketFactory() {
    try {
      String keyStorePath = environment.getProperty("cwa-verification-server.ssl.key-store");
      String keyStorePassword = environment
        .getProperty("cwa-verification-server.ssl.key-store-password");
      String keyPassword = environment.getProperty("cwa-verification-server.ssl.key-password");

      String trustStorePath = environment.getProperty("cwa-verification-server.ssl.trust-store");
      String trustStorePassword = environment
        .getProperty("cwa-verification-server.ssl.trust-store-password");
